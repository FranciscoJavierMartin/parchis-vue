<template>
  <div class="game-logo">
    <div class="game-logo-name">
      <div class="game-logo-dices">
        <DiceFace v-for="i in 2" :key="i" :value="6" :size="40" />
      </div>
      <div v-for="{ letter, color } in DATA_LOGO" :key="letter" class="game-logo-letters">
        <div class="game-logo-letter">{{ letter }}</div>
        <TokenPiece :color="color" :style="{ height: '30px', width: '30px' }" />
      </div>
    </div>
    <div class="game-logo-footer">Party</div>
  </div>
</template>
...
.game-logo {
  ...
  .game-logo-footer {
    ...
    -webkit-text-stroke-color: #7f580d;
  }
}
</style>
